...
if (document.body.id === 'fightPage') {
    ...
    fightButton.addEventListener('click', function () {
        ...
        let playerLog = `You attacked the enemy's ${playerAttackZone} for ${playerDamage} damage.`;
        if (defendedPlayer) playerLog += ` (but it was defended, reduced from ${basePlayerDamage})`;
        addLog(playerLog);

        let enemyLog = `Enemy attacked your ${enemyAttackZone} for ${enemyDamage} damage.`;
        if (defendedEnemy) enemyLog += ` (but it was defended, reduced from ${baseEnemyDamage})`;
        addLog(enemyLog);
